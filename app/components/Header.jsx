...
import { INSTAGRAM_LINK } from "../data/footer";
...
const SOCIAL_LINKS = [
  ...
  { name: "Instagram", href: INSTAGRAM_LINK, Icon: IconInstagram },
...
export default function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 bg-white transition-shadow duration-300 w-full min-w-0 header-no-scrollbar ${scrolled ? "shadow-md" : ""}`}>
      <div className="flex flex-row md:flex-row bg-[#333] text-white text-[10px] sm:text-xs md:text-sm px-3 sm:px-4 lg:px-8 py-1.5 md:py-2 justify-between md:justify-between items-center gap-2 overflow-x-hidden min-w-0">
        <div className="flex items-center gap-2 sm:gap-4 md:gap-6 min-w-0 overflow-hidden">
          ...
          <a href={`mailto:${CONTACT_INFO.emails[0]}`} className="hidden sm:flex items-center gap-1.5 hover:opacity-90 text-white min-w-0 shrink-1">
            ...
            <span className="truncate text-[10px] sm:text-xs md:text-sm">{CONTACT_INFO.emails[0]}</span>
          </a>
        </div>
        <div className="flex gap-2.5 sm:gap-4 items-center shrink-0">
          {SOCIAL_LINKS.map(({ name, href, Icon }) => (
            <a key={name} href={href} target="_blank" rel="noopener noreferrer" className="text-white hover:text-[#E6C76A] transition-colors" aria-label={name}>
              <Icon className="w-3.5 h-3.5 sm:w-5 sm:h-5" />
            </a>
          ))}
        </div>
      </div>
      <nav className="relative z-10 grid grid-cols-[1fr_auto] md:grid-cols-[1fr_auto_1fr] items-center gap-2 px-4 py-3 lg:px-8 min-w-0 overflow-visible">
        <Link href="/" className="flex items-center gap-2.5 sm:gap-3 shrink-0 group min-h-0 justify-self-start">
          <h1 className="m-0 text-lg md:text-xl lg:text-2xl font-bold text-[#333] tracking-tight leading-none whitespace-nowrap self-center">
            Rajasthan Stone Gallery
          </h1>
        </Link>

        <div className="hidden md:flex items-center justify-center gap-6 lg:gap-8 justify-self-center">
          {navItems.map((item) => (
        ...
        <div className="flex items-center gap-4 justify-end justify-self-end">
          <button onClick={toggleMenu} className="md:hidden p-2 -mr-1" aria-label="Toggle menu">
            <div className="w-6 h-5 flex flex-col justify-between">
              <span className={`block h-0.5 bg-black transition-all duration-300 ${isOpen ? "rotate-45 translate-y-2" : ""}`} />
              <span className={`block h-0.5 bg-black transition-all duration-300 ${isOpen ? "opacity-0" : ""}`} />
              <span className={`block h-0.5 bg-black transition-all duration-300 ${isOpen ? "-rotate-45 -translate-y-2" : ""}`} />
            </div>
          </button>
        </div>
      ...
      <p className="relative z-0 text-center py-2 font-medium text-[#111] text-sm sm:text-base animate-fade-blink">More Than Marble. A Legacy the World Trusts.</p>
    </header>
  );
}
